<script lang="ts">
	...
	let {
		value = $bindable(''),
		placeholder = m['SearchFeature.SearchInput.searchPlaceholder'](),
		label = placeholder,
		onclear,
		dropdown,
		dropdownOpen = true,
	...
	<InputGroup.Root class="group">
		...
		<InputGroup.Addon align="inline-end">
			<InputGroup.Button
				...
				size="icon-xs"
				class={cn(
					'rounded-full text-muted-foreground transition-none hover:text-foreground active:not-aria-[haspopup]:translate-y-0',
				...
				aria-label={m['SearchFeature.SearchInput.clearSearch']()}
				disabled={!value || disabled}
				title={m['SearchFeature.SearchInput.clearSearch']()}
			>
