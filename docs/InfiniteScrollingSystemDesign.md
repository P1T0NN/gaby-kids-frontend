...
The current project uses the product aggregate for maintained unfiltered
product totals and reads the value through `getTotalSizeAggregate`. The same
pattern is appropriate for an unfiltered accommodation total, provided the
counter is scoped correctly and all writes update it reliably.
...
3. Implement `InfiniteScroll.svelte` with a Svelte 5 attachment,
   ...
   (Complete.)
4. Create one integration using the existing Convex query wrapper.
...
7. Add exact filtered counts only when the product requires them and the
   ...
   strategy. Search remains explicitly uncounted.
